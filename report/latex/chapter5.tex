...
\begin{enumerate}
\item \textbf{Sample size.} The paired analysis rests on $n=4$ students per environment. This may be enough to detect very large effects (Open-Mindedness, $d>1.6$ throughout) but underpowered for the medium effects seen in Conscientiousness, and it limits the Wilcoxon test to a small set of possible $p$-values, several of which never reach $p<0.05$ regardless of effect size.
