...
For each of the five party environments, we compare the pre- and post-simulation BFI-2 interview responses for the four student agents using a paired Wilcoxon signed-rank test. Table~\ref{tab:paired-stats} shows the mean shift ($\Delta$) in each domain, the standard deviation of that shift ($sd_{\Delta}$), the paired Cohen's $d$ effect size, and the $p$-value from the Wilcoxon test ($p_t$). The table also indicates which shifts are statistically significant at the $p<0.05$ level.
